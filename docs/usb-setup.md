...
光の調整（ハイライト/シャドウトーン）、色のバランス（WBシフト2軸・色温度）、
クラリティやグレインといった項目も、カメラが公開していれば同様に解決されます。
公開されていなければ「解決できなかったフィールド」に並び、適用時はスキップされます。
...
| ハイライト/シャドウトーンやWBシフトが「非対応」になる | libgphoto2 がそのプロパティを公開していない可能性。`config/gphoto2-mapping-raw.txt` を `Highlight` `Shadow` `Adjust` などで検索し、見つかった名前を `src/camera/gphoto2/mapping.ts` の candidates に追加する |
