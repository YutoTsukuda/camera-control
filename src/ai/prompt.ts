...
ハイライトトーンとシャドウトーンは -2〜+4（0.5刻み）。**マイナスが軟調、プラスが硬調**。

- ハイライトトーン マイナス: 明部の階調を粘らせる。白飛びを避けたいとき（雪、白い壁、逆光の空、ウェディング）
- ハイライトトーン プラス: 明部を締めて抜けを良くする。眠い光をはっきりさせたいとき
- シャドウトーン マイナス: 暗部を持ち上げる。黒が浅くなるが情報は残る（逆光の人物、日陰、室内）
- シャドウトーン プラス: 黒を締める。締まりを出したいとき（夜景、モノクロ、ネオン）

輝度差が大きいシーンでは「ハイライト マイナス ＋ シャドウ マイナス」で全体を軟調に寄せ、
平坦な曇天では「ハイライト プラス ＋ シャドウ プラス」でメリハリを付ける、が基本形。
ただし両方を大きく振ると不自然になるので、±1〜2 の範囲で収める。

ダイナミックレンジ設定（DR200/DR400）は白飛びを救うための別の手段。
トーンカーブは階調の性格を、DR は救える範囲を変える。混同しない。

# 色のバランス（WBシフト）

X100VI には R/G/B を独立に上げ下げする機構は無い。色を追い込む手段は 2 つ。

1. **ホワイトバランス**（プリセットまたは色温度指定 2500〜10000K）で基準を決める
2. **WBシフト**で微調整する。2 軸あり、それぞれ -9〜+9:
   - wbShiftRed: プラスで赤方向、マイナスでシアン方向
   - wbShiftBlue: プラスで青方向、マイナスで黄方向

2 軸の組み合わせで斜め方向も出せる:
   - 緑かぶり（蛍光灯・木漏れ日）を打ち消す → 赤プラス ＋ 青プラス（マゼンタ方向）
   - マゼンタかぶりを打ち消す → 赤マイナス ＋ 青マイナス（グリーン方向）

実用的な当て方:
   - 夕景・朝焼けの赤みを強調 → 赤 +2〜+4
   - 曇天や日陰の青かぶりを戻す → 赤 +1〜+2
   - 白熱電球の橙を残して雰囲気を出す → WBをオート（雰囲気優先）にするか、赤 +1
   - 蛍光灯の緑かぶり → 赤 +2 ＋ 青 +1 程度
   - ネオンの色を忠実に出したい → シフトは 0 のままにして、WBを色温度指定で固定する

シフトは効きが強いので、確信が無いときは 0 のままにする。
外した色かぶりは後から直せるが、過補正は不自然さとして残る。
