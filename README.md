...
決めるのは露出だけではありません。フィルムシミュレーション、
**光の調整（ハイライト/シャドウトーン）**、**色のバランス（WBシフト2軸・色温度）**、
クラリティ、グレイン、ダイナミックレンジまで含めて 1 タップで反映します。
...
| 分類 | 項目 |
|---|---|
| 露出 | 絞り / シャッター速度 / ISO / 露出補正 / NDフィルター / シャッター方式 / 露出モード |
| 絵作り | フィルムシミュレーション（20種）/ ダイナミックレンジ / Dレンジ優先 |
| 光の調整 | ハイライトトーン / シャドウトーン（-2〜+4、マイナスが軟調） |
| 色のバランス | ホワイトバランス / 色温度（2500〜10000K）/ WBシフト 赤軸・青軸（各 -9〜+9） |
| 質感 | カラー（彩度）/ シャープネス / ノイズリダクション / クラリティ / グレイン / カラークローム |
| 操作 | 測光 / フォーカス / AFエリア / 被写体検出 / ドライブ / 画質 / デジタルテレコン |

> **RGBを独立に操作することはできません。** X100VI に R/G/B を個別に上げ下げする機構が
> 無いためです。色を追い込む手段は「ホワイトバランス（または色温度指定）で基準を決め、
> WBシフトの2軸で微調整する」の 2 段構え。赤軸と青軸を組み合わせることで
> マゼンタ方向（緑かぶりの打ち消し）やグリーン方向も出せます。
> AI にはこの制約と当て方をプロンプトで明示しているので、存在しない操作は提案しません。

実際にカメラへ書き込めるのは、**そのカメラが gphoto2 経由で公開している項目だけ**です。
公開されていない項目は「非対応」として理由つきで報告され、送信されません。
何が書き込めるかは `npm run probe` が教えてくれます。
